...
function GetCalendarHTML(month,year)
{
  ...
  result+='    <TABLE BORDER=0 CELLSPACING=1 CELLPADDING=1 class=2 style="width: 240px;">\n';
  result+='      <TR ALIGN=CENTER class="footer">\n';
  result+='        <TD CLASS="FUENTE2 texto_general_black">D</TD>\n';
  result+='        <TD CLASS="FUENTE  texto_general_black">L</TD>\n';
  result+='        <TD CLASS="FUENTE  texto_general_black">M</TD>\n';
  result+='        <TD CLASS="FUENTE  texto_general_black">M</TD>\n';
  result+='        <TD CLASS="FUENTE  texto_general_black">J</TD>\n';
  result+='        <TD CLASS="FUENTE  texto_general_black">V</TD>\n';
  result+='        <TD CLASS="FUENTE2 texto_general_black">S</TD>\n';
  result+='      </TR>\n';
  ...
  for (var i=0; i<6 && day<=monthDays;i++)
  {
    ...
    for (var j=0;j<7;j++)
    {
      if (day>=1 && day<=monthDays)
      {
        ...
        {
	          result+='        <TD class="' + sClass + '" WIDTH=28><A CLASS="dia_calendario" HREF="javascript:SelectDate(' + month + ',' + day + ',' + year + ');"><B>'+day+'</B></A></TD>\n';
        }  
        else
	          result+='        <TD class="' + sClass + '" WIDTH=28><A CLASS="dia_calendario" HREF="javascript:SelectDate(' + month + ',' + day + ',' + year + ');">'+day+'</A></TD>\n';
      }
      else
...
<table border=0 cellpadding=0 cellspacing=0 class="1 controles_calendario" width=100%>
<tr>
<FORM NAME="calendar_control" action="win_calendar.jsp">
  <td align="right" STYLE="height:32px; padding-left: 15px; text-align: left;">
  
	<input type="hidden" name="wNombreForm" value="<%=wNombreForm%>">
	<input type="hidden" name="wNombreField" value="<%=wNombreField%>">
    <SELECT NAME="month" ONCHANGE="javascript:document.calendar_control.submit();" class="pageinfo lp-select-calendar-month">
  ...
  <td align="center" STYLE="height:32px;">
    <SELECT NAME="year" ONCHANGE="javascript:document.calendar_control.submit();" class="pageinfo lp-select-calendar-year">
      <%
  ...
  <td align="left" STYLE="height:32px; padding-right: 15px; text-align: right;">
    <INPUT TYPE=BUTTON ONCLICK="parent.close()" VALUE="Cancelar" class="pageinfo lq-btn lp-glyphicon lp-glyphicon-close-blanco iconCancelarCalendar">   
  </td>
</tr>
</table>
<BR/>
